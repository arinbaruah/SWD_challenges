...
pl <- ggplot(data = df_cricket_pca,aes(x = PC1,y =PC2,text = Player)) + geom_point(color = "lightgray")  + 
  geom_label_repel(data = filter(df_cricket_pca, Player %in% top_players), 
             aes(label = Player),nudge_y = 0.85,nudge_x = -0.85) + geom_point(data = filter(df_cricket_pca, Player %in% top_players), 
             color = "darkred",size =4) +
  labs(x = "PC1 (Batting qualities)",y = "PC2 (Bowling qualities)",title = " Who are the top performing players in the Australian women's cricket team?") + 
  ...
  annotate("text", x = -9, y= -8, 
   ...
   annotate("text", x = -4.5, y= 0, 
           ...
           size = unit(5, "pt")) +
    theme_minimal() +
  theme(plot.background = element_rect(fill = "cornsilk"),plot.title = element_text(face = "bold")) 
